"""
tcp socket for p2p communication without central server
this socket is used for node of Chord DHT

    - 실행
        root노드 : python p2p_tcp.py 8000(내port)
        다른 노드 : python p2p_tcp.py 7000(내port) 8000(root node port)
"""
import socket, time
import threading
import hashlib
...
class P2PNode:
    ...
    def __init__(self, addr, host_addr=None):
        self.addr = addr
...
if __name__ == '__main__':
    ...
    if args.log == True:
        ...
        log_handler.setFormatter(formatter)
        logger.addHandler(log_handler)
    node = P2PNode((this_addr), (help_addr))